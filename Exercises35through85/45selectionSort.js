function selectionSort(arr, comparator){
    ...
    for (let j = 0; j < arr.length; j++){    //loop thru the entire array
        min = arr[j];
        for (var i = start; i < arr.length; i ++){    //loop from the first number not replaced by a minimumm yet
            if(comparator(arr[i], min) < 1) {
                min = arr[i];
                placeHolder = i;
            }
         }//end for i
